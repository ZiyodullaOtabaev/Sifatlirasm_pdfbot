...
def db_init():
    ...
    with db_connect() as con:
        ...
        """)
        cur.execute("""
        CREATE TABLE IF NOT EXISTS channel_user_log (
...
def get_active_channels() -> list[dict]:
    ...
    with db_connect() as con:
        rows = con.execute("""
            SELECT id, channel_id, channel_title, invite_link, target_subs, current_subs, is_active
...
def get_all_channels() -> list[dict]:
    """Get all required channels (active and inactive)."""
    with db_connect() as con:
        rows = con.execute("""
            SELECT id, channel_id, channel_title, invite_link, target_subs, current_subs, is_active
            FROM required_channels
...
def add_required_channel(channel_id: str, channel_title: str, invite_link: str, target_subs: int = 0) -> int:
    """Add a new required channel with optional subscriber target limit."""
    with db_connect() as con:
        cur = con.execute("""
            INSERT INTO required_channels (channel_id, channel_title, invite_link, target_subs, current_subs, is_active)
            VALUES (?, ?, ?, ?, 0, 1)
            ON CONFLICT(channel_id) DO UPDATE SET
                channel_title = excluded.channel_title,
                invite_link = excluded.invite_link,
                target_subs = excluded.target_subs,
                is_active = 1
        """, (channel_id, channel_title, invite_link, target_subs))
        con.commit()
        return cur.lastrowid


def delete_required_channel(channel_id: str):
    """Delete a required channel by channel_id or ID."""
    with db_connect() as con:
        if str(channel_id).isdigit():
            con.execute("DELETE FROM required_channels WHERE id = ?", (int(channel_id),))
        else:
            con.execute("DELETE FROM required_channels WHERE channel_id = ?", (channel_id,))
        con.commit()


def toggle_channel_status(channel_id: str, is_active: int):
    """Toggle active state of a channel."""
    with db_connect() as con:
        if str(channel_id).isdigit():
            con.execute("UPDATE required_channels SET is_active = ? WHERE id = ?", (is_active, int(channel_id)))
        else:
            con.execute("UPDATE required_channels SET is_active = ? WHERE channel_id = ?", (is_active, channel_id))
        con.commit()


def record_channel_join(channel_id: str, user_id: int) -> tuple[bool, bool, int, int]:
    """
    Record that a user has verified / joined a required channel.
    Returns: (is_new_join, is_target_reached, current_subs, target_subs)
    """
    with db_connect() as con:
        # Check if already logged for this user
        existing = con.execute(
            "SELECT 1 FROM channel_user_log WHERE channel_id = ? AND user_id = ?",
            (channel_id, user_id)
        ).fetchone()
        ...
        # Log user join
        con.execute(
            "INSERT OR IGNORE INTO channel_user_log (channel_id, user_id, joined_at) VALUES (?, ?, datetime('now'))",
            (channel_id, user_id)
        )

        # Increment current_subs
        con.execute(
            "UPDATE required_channels SET current_subs = current_subs + 1 WHERE channel_id = ?",
            (channel_id,)
        )

        row = con.execute(
            "SELECT target_subs, current_subs, is_active FROM required_channels WHERE channel_id = ?",
            (channel_id,)
        ).fetchone()
        ...
        current = row["current_subs"]
        target_reached = False

        # If target is set (>0) and reached -> Auto-detach!
        if target > 0 and current >= target:
            con.execute("UPDATE required_channels SET is_active = 0 WHERE channel_id = ?", (channel_id,))
            con.commit()
            target_reached = True

        return (True, target_reached, current, target)
