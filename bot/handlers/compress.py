...
from bot.states import get_state, set_state, STATE_WAIT_COMPRESS_PDF, STATE_NONE
...
async def handle_compress_pdf(message: Message, bot: Bot):
    ...
    status = await message.answer(t("compress_pdf_generating", lang))
    ...
    try:
        ...
        caption = (
            f"{t('compress_pdf_ready', lang)}\n\n"
            f"📁 {original_size // 1024} KB ➡️ 📦 <b>{compressed_size // 1024} KB</b> ({saved_pct}% tejaldi)"
        )
        await bot.send_document(user_id, result, caption=caption, parse_mode="HTML")
